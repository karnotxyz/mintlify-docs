...
description: "How credits determine each holder's share of points across periods, using capital multiplied by time."
icon: "calculator"
---

Credits determine your share of points. Points are distributed period by period, not as a single lump sum at the end of a series. This page explains how credits work, why periods exist, and how GCI lets the protocol track credits on-chain.
...
Without credits, points distribution would be trivially gameable. A naive design that distributes points proportional to token balance right before points are distributed lets anyone deposit one second before distribution and claim the same share as a day-one holder.
...
Each series is divided into equal-sized **credit periods**. These periods are chosen to match the point distribution frequency on the underlying exchange.

So if an exchange distributes points every week, the protocol uses one-week credit periods on-chain.

> A credit period is the on-chain period whose accumulated credits receive the points earned for the corresponding exchange period.

---

## Why Periods Are Shifted

There is one important nuance: on-chain periods are shifted by a fixed amount relative to the exchange periods.
...
So the period tracked on-chain is shifted relative to the period used off-chain by the exchange.
...
In this example, when an on-chain period ends, the corresponding off-chain exchange points are received one day later and distributed back to that on-chain period.

---

## How Credits Are Calculated

Credits are a way to track what share of total points you should receive at the end of every period.

From the protocol's perspective, the fair way to allocate points is to measure:
...
## Period-by-Period Points Distribution

Points are distributed period by period:

$$
\text{yourPoints} = \sum_{p=1}^{P} \frac{\text{yourCredits}_p}{\text{totalCredits}_p} \times \text{points}_p
$$
...
- $\text{points}_p$ = points assigned to that period

This is why early participation matters. If it is easier to get points in the early days of an exchange, early holders get that upside. Later entrants do not dilute those earlier periods.
...
- Weeks 1-6 distribute 800 points per week
- Weeks 7-10 distribute 200 points per week
...
| Week | Alice total credits | Bob total credits | Alice's share of points |
|------|---------------------|-------------------|-------------------------|
...
Even though Bob contributes the same total capital x time as Alice, Alice gets far more points because the earlier weeks were richer and those periods are locked in to the early holder.
